...
@allure.epic("场景1")  # 测试报告：特性场景名称
@allure.feature("登录模块")  # 测试报告：模块名
class Test01:
    filename = os.path.join(config_path.test_data_dir, "test_case_login.xlsx")  # 获取测试数据路径
    ...
    def test01(self, datas, func):
        ...
        try:
            logging.info(f"开始执行用例：{case_name}")  # 打印日志
